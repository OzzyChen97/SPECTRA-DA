...
from labeled source simulations. This step is the strongest assumption in the
method: the source-simulation bank must contain covariance corrections that
are relevant to the real target, and the unlabeled descriptors must identify
them reliably. We construct pseudo-target graphs
\begin{equation}
...
\subsection{Practical recovery and reliability accounting}
...
The frozen selector uses a conservative source-only uncertainty term rather
than treating transported covariance as always trustworthy. Bootstrap
resampling of simulated shifts yields risk estimates
...
from covariance transport in Table~\ref{tab:core_ablation}; in the current
frozen run it is a guardrail, not the dominant source of gain.

The failure mode suggests a stronger reliability criterion for future
selectors. Let $U_m^{\mathrm{tr}}$ measure disagreement among plausible
transported covariance corrections or their induced risk-correction vectors,
and let $T_m$ be an independent transferability score such as Transfer Score.
The underlying conservative principle is
\begin{equation}
  \operatorname{score}_m
  = \widehat R_m+\lambda U_m^{\mathrm{tr}}-\gamma T_m,
  \label{eq:reliable_selection}
\end{equation}
where aggressive covariance correction is penalized when transport is
unstable and an independent transferability prior can veto an overconfident
top prediction. The released evaluation scaffold uses the rank-normalized
pre-registered version of this rule,
\begin{equation}
  s_m=(1-\eta)\operatorname{rank}(\widehat R_m)/\tau
  +\lambda\operatorname{rank}(U_m^{\mathrm{tr}})
  +\gamma\operatorname{rank}(-T_m),
  \label{eq:rank_reliable_selection}
\end{equation}
with only $\lambda,\gamma,\eta,\tau$ allowed to vary under source-family
holdout. We do not report this as a completed result; it is the direct design
implication of the real-target failure analysis in
