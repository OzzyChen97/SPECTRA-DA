\section{Covariance-Aware Spectral Risk Recovery}
\label{sec:method}

\subsection{Problem setting and sealed selection}

We study model selection after UGDA training. \method is the paper's
risk-recovery research module within the broader \benchmark protocol; it is
not the promoted outcome of the final audit. Let
...
prior is disabled when the target descriptor falls outside the pre-registered
source-simulation support. The research implementation additionally winsorizes
...
The source-simulated configuration uses a conservative source-only uncertainty
term rather than treating transported covariance as always trustworthy. Bootstrap
resampling of simulated shifts yields risk estimates
$\widehat R_T^{(q)}(h_m)$ and uncertainty
$u_m=\operatorname{Std}_q[\widehat R_T^{(q)}(h_m)]$. The research score uses
\begin{equation}
  \widehat m=\arg\min_m
...
from covariance transport in Table~\ref{tab:core_ablation}; it is a guardrail,
not the dominant source of gain. The subsequent audit tests fixed covariance
shrinkage, pair-consistency trust, rank consensus, and bootstrap stability as
ways to decide whether this correction should influence real-target
selection. None passes all registered promotion conditions. Consequently,
\method remains a recovery and diagnostic module rather than a frozen
deployment selector.
