"""Upstream ADAlign model internals used only by the sealed trajectory adapter.

The original ``fit`` and ``predict`` methods below read labels from the graph
passed to them. SPECTRA-DA never calls those methods for target data; the safe
adapter in ``scripts/trajectory_export/run_adalign.py`` trains and exports
predictions through a label-free target graph instead.
"""
